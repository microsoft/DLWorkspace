...
def calculate_vc_resources(cluster_capacity, cluster_available,
                           cluster_reserved, vc_info, vc_usage):
    logger.debug("cluster_capacity %s, cluster_available %s, "
                 "cluster_reserved %s",
                 cluster_capacity, cluster_available, cluster_reserved)
    logger.debug("vc_info %s, vc_usage %s", vc_info, vc_usage)
    vc_total = collections.defaultdict(lambda: ClusterResource())
    vc_used = collections.defaultdict(lambda: ClusterResource())
    vc_available = collections.defaultdict(lambda: ClusterResource())
    vc_unschedulable = collections.defaultdict(lambda: ClusterResource())

    vc_quota_sum = 0
    for vc_name, vc_quota in vc_info.items():
        vc_total[vc_name] = vc_quota
    ...
    # key is vc_name, value is a ClusterResource object
    ratio = collections.defaultdict(lambda: {})
    ...
    for vc_name, gpu_info in list(vc_info.items()):
        for gpu_type, quota in list(gpu_info.items()):
            if vc_quota_sum == 0:
                ...
                unschedulable = float(cluster_reserved.get(
                    gpu_type, 0)) * quota / vc_quota_sum
            vc_quota = quota - int(math.ceil(unschedulable))
