...
class DataHandler:
    ...
    def AddStorage(self, vcName, url, storageType, description, defaultMountPath):
        try:
            start_time = timeit.default_timer()
            sql = "INSERT INTO [%s] (storageType, url, description, vcName, defaultMountPath) VALUES (?,?,?,?,?)""" % self.storagetablename
            cursor = self.conn.cursor()
    ...
    def AddVC(self, vcName, resources):
        try:
            start_time = timeit.default_timer()
            sql = """INSERT INTO [%s] (vcName, resources) VALUES (?,?)""" % self.vctablename
            cursor = self.conn.cursor()
    ...
    def UpdateIdentityInfo(self, identityName, identityId, groups):
        try:
            ...
            if len(self.GetIdentityInfo(identityName)) == 0:
                sql = """INSERT INTO [%s] (identityName,identityId,groups) VALUES (?,?,?)""" % self.identitytablename
