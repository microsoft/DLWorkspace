...
def AddStorage(userName, vcName, url, storageType, description, defaultMountPath):
    ret = None
    dataHandler = DataHandler()
    if AuthorizationManager.IsClusterAdmin(userName):
        ret =  dataHandler.AddStorage(vcName, url, storageType, description, defaultMountPath)
    else:
        ret = "Access Denied!"
    dataHandler.Close()
    return ret
...
def UpdateStorage(userName, vcName, url, storageType, description, defaultMountPath):
    ret = None
    dataHandler = DataHandler()
    if AuthorizationManager.HasAccess(userName, ResourceType.VC, vcName, Permission.Admin):
        ret =  dataHandler.UpdateStorage(vcName, url, storageType, description, defaultMountPath)
    else:
        ret = "Access Denied!"
    dataHandler.Close()
    return ret
...
def AddVC(userName, vcName, resources):
    ret = None
    dataHandler = DataHandler()
    if AuthorizationManager.IsClusterAdmin(userName):
        ret =  dataHandler.AddVC(vcName, resources)
    else:
        ret = "Access Denied!"
    dataHandler.Close()
    return ret
...
def UpdateVC(userName, vcName, resources):
    ret = None
    dataHandler = DataHandler()
    if AuthorizationManager.IsClusterAdmin(userName):
        ret =  dataHandler.UpdateVC(vcName, resources)
    else:
        ret = "Access Denied!"
    dataHandler.Close()
    return ret
