...
default_config_mapping = {
    ...
    "api-server-ip": (["service_cluster_ip_range"],
                      lambda x: generate_ip_from_cluster(x, 1)),
    "dns-server-ip": (["service_cluster_ip_range"],
                      lambda x: generate_ip_from_cluster(x, 53)),
    "network-trusted-domains":
        (["network"], lambda x: generate_trusted_domains(x, 5)),
    # master deployment scripts
    "premasterdeploymentscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "pre-master-deploy.sh"),
    "postmasterdeploymentscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "post-master-deploy.sh"),
    "mastercleanupscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "cleanup-master.sh"),
    "masterdeploymentlist":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "deploy.list"),
    # worker deployment scripts
    "preworkerdeploymentscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "pre-worker-deploy.sh"),
    "postworkerdeploymentscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "post-worker-deploy.sh"),
    "workercleanupscript":
        (["platform-scripts"],
         lambda x: get_platform_script_directory(x) + "cleanup-worker.sh"),
...
def gen_platform_wise_config(config):
    ...
    platform_func = {
        'azure_cluster': load_az_params_as_default,
...
def add_ssh_key(config):
    ...
    if isinstance(keys, list):
        if "sshkey" in config and "sshKeys" in config and not (
                config["sshkey"] in config["sshKeys"]):
...
def get_domain(config):
    if "network" in config and "domain" in config["network"] and len(
            config["network"]["domain"]) > 0:
...
def load_node_list_by_role_from_config(config, roles, with_domain=True):
    ...
    for role in roles:
        assert role in config[
            "allroles"], "invalid role, check your list of valid roles in config"
...
def load_default_config(config):
    ...
    config["admin_username"] = config.get(
        "admin_username",
    ...
    config["restapi"] = "http://%s:%s" % (config["kubernetes_master_node"][0],
                                          config["restfulapiport"])
    if os.path.isfile(config["ssh_cert"] + ".pub"):
        with open(config["ssh_cert"] + ".pub") as f:
            config["sshkey"] = f.read()
    config["files2cp"] = {
        "common": [
            "./cloud_init_mkdir_and_cp.py", "./scripts/prepare_vm_disk.sh",
            "./scripts/prepare_ubuntu.sh",
            "./scripts/disable_kernel_auto_updates.sh",
            "./scripts/docker_network_gc_setup.sh", "./deploy/scripts/dns.sh",
            "./deploy/etcd/init_network.sh", "scripts/render_env_vars.sh",
            "scripts/fileshare_install.sh", "scripts/mnt_fs_svc.sh",
            "scripts/build_lustre_client.sh"
        ],
        "infra": [
            "./deploy/master/" + config["premasterdeploymentscript"],
            "./deploy/master/" + config["postmasterdeploymentscript"],
            "./scripts/cloud_init_infra.sh",
            "deploy/cloud-config/infra.kubelet.service.template",
            "deploy/scripts/pass_secret.sh", "deploy/services"
        ],
        "worker": [
...
def create_cluster_id(overwrite=False):
    ...
    if ID is None or overwrite:
        ...
        print(
            'Cluster ID file exists -- ./deploy/clusterID.yml:\n{}'.format(ID))
        return ID
...
def load_config(args):
    ...
    load_node_list_by_role_from_config(config, [
        "infra", "worker", "nfs", "etcd", "kubernetes_master", "elasticsearch",
        "lustre", "mdt", "oss"
    ])
    ...
    if not "private_docker_registry" in config or not "cloudinit" in config[
            "private_docker_registry"]:
        print(
            "mush specify a private docker registry to put the cloudinit docker that contains sensitive information of this cluster!"
        )
        exit(-1)
    return config
...
def GetCertificateProperty(config):
    ...
    config["apiserver_ssl_ip"] = "\n".join([
        "IP.{} = {}".format(i, sslip)
        for i, sslip in enumerate([config["api-server-ip"]] +
                                  config["ssl_localhost_ips"] + masterips)
    ])
    ...
    config["etcd_ssl_ip"] = "\n".join([
        "IP.{} = {}".format(i, sslip)
        for i, sslip in enumerate(config["ssl_localhost_ips"] + etcdips)
    ])
    return config
...
def get_easy_rsa_and_cfssl(config):
    ...
    copy_from_docker_image(
        config["dockers"]["container"]["binstore"]["fullname"],
        "/data/easy-rsa/v3.0.5.tar.gz",
        "./deploy/bin/other/easy-rsa/v3.0.5.tar.gz")
    copy_from_docker_image(
        config["dockers"]["container"]["binstore"]["fullname"],
        "/data/cfssl/linux/cfssl", "./deploy/bin/other/cfssl")
    copy_from_docker_image(
        config["dockers"]["container"]["binstore"]["fullname"],
...
def gen_mounting_yaml(config):
    ...
    for node_fqdn in config.get("nfs_node", []) + config.get("mdt_node", []):
        ...
        assert "private_ip" in spec, "Need IP for NFS or MDT node {}!".format(
            storage_node_name)
        file_shares = []
        if "fileshares" not in spec or len(spec["fileshares"]) == 0:
            spec["fileshares"] = [[]]
        for v in spec["fileshares"]:
            server_path = v["server_path"] if "server_path" in v else config[
                "nfs-mnt-src-path"]
            ...
            if "client_mount_root" in v:
                mnt_root = os.path.join(
                    v["client_mount_root"],
                    vc) if "vc" in v else v["client_mount_root"]
            else:
                mnt_root = config[
                    "physical-mount-path-vc"] if "vc" in v else config[
                        "physical-mount-path"]
            ...
            if "client_link_root" in v:
                ...
                lnk_root = config[
                    "dltsdata-storage-mount-path"] if "vc" in v else config[
                        "storage-mount-path"]
            # process leaves
            ...
            for link_pair in v["client_links"]:
                ...
                file_share_item["client_links"].append({
                    "src": link_pair["src"],
                    "dst": link_dst
                })
            file_shares.append(file_share_item)
        options = spec["options"] if "options" in spec else config[
            "mountconfig"]["nfs"]["options"]
...
def get_kube_labels_of_machine_name(config, node_name):
    ...
    for grp in default_grp:
        default_labels.update(config["default_kube_labels_by_node_role"].get(
            grp, {}))
    default_labels.update(config["machines"][node_name].get("kube_labels", {}))
    return [
        ky.replace("/", "\\/") + '=' + val.replace("/", "\\/")
        for ky, val in default_labels.items()
    ]
...
def render_infra_node_specific(config, args):
    ...
    service_list = config["machines"][hostname].get(
        "kube_services", config["kube_services_2_start"])
    service_list = list(
        set(service_list + add_implied_services_based_on_config(config)))
    ...
    config["file_modules_2_copy"] = [
        "kubernetes_common", "kubernetes_infra", "etcd", "ip_resolve",
        "restfulapi", "nfs_client", "repairmanager"
    ]
    utils.render_template(
        "./template/cloud-config/cloud_init_infra.txt.template",
...
def render_worker_node_specific(config, args):
    ...
    for sku in config["worker_sku_cnt"]:
        ...
        need_infiniband = config.get("sku_mapping",
                                     {}).get(sku, {}).get("infiniband", False)
        if need_infiniband:
            config["script_modules"].append("infiniband")
        gpu_type = config.get("sku_mapping",
                              {}).get(sku, {}).get("gpu-type", "None")
        ...
        utils.render_template(
            ...
            "./deploy/cloud-config/cloud_init_worker_{}.txt".format(sku),
            config)
...
def render_nfs_node_specific(config, args):
    ...
    for nfs in config.get("nfs_node", []):
        ...
        config["data_disk_mnt_path"] = config["machines"][nfs_machine_name][
            "data_disk_mnt_path"]
        config["files2share"] = []
        ...
        config["storage_manager"] = config["machines"][nfs_machine_name][
            "storage_manager"]
        ...
        utils.render_template(
            "./template/cloud-config/cloud_init_nfs.txt.template",
            "./deploy/cloud-config/cloud_init_{}.txt".format(nfs.split(".")[0]),
            config)
...
def render_mdt_node_specific(config, args):
    ...
    utils.render_template(
        "./template/cloud-config/cloud_init_lustre.txt.template",
...
def render_oss_node_specific(config, args):
    ...
    for oss in config.get("oss_node", []):
        ...
        if not "data_disk_mnt_path" in oss_spec:
            ...
            for disk_item in oss_spec["managed_disks"]:
                ...
                for i in range(disk_item["disk_num"]):
                    oss_spec["data_disk_mnt_path"].append(
                        "/lustre-oss{0:02d}".format(lustre_disk_id + i))
                lustre_disk_id += disk_item["disk_num"]
        if isinstance(oss_spec["data_disk_mnt_path"], list):
            oss_spec["data_disk_mnt_path"] = ';'.join(
                oss_spec["data_disk_mnt_path"])
        ...
        utils.render_template(
            "./template/cloud-config/cloud_init_lustre.txt.template",
            "./deploy/cloud-config/cloud_init_{}.txt".format(oss_machine_name),
            config)
    return config
...
def render_ETCD(config):
    ...
    config["etcd_node_num"] = len([
        spec for spec in config["machines"].values() if 'etcd' in spec['role']
    ])
...
def check_buildable_images(image_list, config):
    for imagename in image_list:
        ...
        if imagename in config["build-docker-via-config"]:
            print("Docker image {} should be built via configuration. ".format(
                imagename))
            exit()
...
def login_private_docker(config):
    for docker_registry, credential in config[
            "private_docker_credential"].items():
...
def get_services_path_list(service_names):
    ...
    for service in service_names:
        ...
        if os.path.exists(os.path.join(dirname,
                                       "launch_order")) and "/" not in service:
    ...
    service_script_paths = [
        "services/" + fn.split('services/')[1]
        for fn in service_script_paths_dpl
    ]
...
def pack_cloudinit_roles(config, args):
    ...
    roles = args.nargs if args.nargs else [
        "common", "infra", "worker", "nfs", "lustre_server"
    ]
    for role in roles:
...
def pack_cloudinit_role(config, args, role):
    if args.force or not os.path.exists(CLOUD_INIT_FILE_MAP):
        with open("./deploy/cloud-config/file_map.yaml") as rf:
            ...
            deploy_files = [
                itm["src"] for mod_val in file_map.values() for itm in mod_val
            ]
            ...
            for mod_name, map_list in file_map.items():
                for map_itm in map_list:
                    de_root_fn = os.path.join(
                        mod_name, os.path.relpath(map_itm["src"],
                                                  start=src_root))
                    os.system('mkdir -p cloudinit/{}'.format(
                        os.path.dirname(de_root_fn)))
                    os.system('cp -r {} cloudinit/{}'.format(
                        map_itm["src"], de_root_fn))
...
def gen_pass_secret_script(config):
    utils.render_template("./template/secret/pass_secret.sh.template",
                          "deploy/scripts/pass_secret.sh", config)
...
def render_storagemanager(config, nodename):
    ...
    with open("./deploy/cloud-config/file_map.yaml") as rf:
        ...
        file_map["{}_storage_manager".format(nodename)] = [{
            "src": deploy_path,
            "dst": "/etc/StorageManager/config.yaml"
        }]
        ...
        if os.path.isdir(dirname):
            ...
            if os.path.isfile(launch_order_file):
                ...
                with open(yamlname) as f:
                    ...
                    if kind in [
                            "Deployment", "DaemonSet", "ReplicaSet", "CronJob",
                            "StatefulSet"
                    ]:
...
def get_service_name(service_config_file):
    try:
        ...
        if not name is None:
            ...
            name = fetch_dictionary(service_config,
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        "--nodes",
        help=
        "Specify an python regular expression that limit nodes that the operation is applied.",
        action="store",
        default=None)
