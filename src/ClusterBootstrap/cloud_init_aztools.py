...
def load_sshkey(config):
    assert os.path.exists('./deploy/sshkey/id_rsa.pub') and "Generate SSHKey first!"
...
def create_nfs_nsg(config, verbose, output_file):
    ...
    merged_ip = keep_widest_subnet(config["cloud_config"]["nfs_ssh"]["source_ips"] + source_addresses_prefixes)
    cmd = """az network nsg rule create \
            --resource-group %s \
            --nsg-name %s \
            --name allow_ssh\
        ...
        for i in range(spec["num"]):
            ...
            if "kube_label_groups" not in spec:
                if "infra" in spec["role"]:
                    cc["machines"][vmname]["kube_label_groups"] = ["all", "etcd_node", "etcd_node_1"]
                elif "worker" in spec["role"]:
                    cc["machines"][vmname]["kube_label_groups"] = ["all", "worker_node"]
...
def add_machine(vmname, spec, verbose, output_file):
    if "pwd" in spec:
        auth = "--authentication-type password --admin-password '{}' ".format(spec["pwd"])
    else:
    ...
    if "private_ip_address" in spec:
        ...
        assert (not 'nfs' in spec["role"]) and "Must specify IP address for NFS node!"
    ...
    if "availability_set" in spec:
        availability_set = "--availability-set '{}'".format(spec["availability_set"])
    elif "worker" in spec["role"] and "availability_set" in config["azure_cluster"]:
        availability_set = "--availability-set '{}'".format(config["azure_cluster"]["availability_set"])
    
    cloud_init = ""
    cloud_init_file = spec.get("cloud_init_file", 'deploy/cloud-config/cloud_init_{}.txt'.format(vmname))
    ...
    if "storage" in spec:
        for st in spec["storage"]:
            if "is_os" in st and st["is_os"]:
                ...
                storage_sku += "os={}".format(st.get("sku", config["azure_cluster"]["os_storage_sku"]))
                os_disk_size_gb = "--os-disk-size-gb " + str(st.get("size_gb", config["azure_cluster"]["os_storage_sz"]))
                ...
                data_disk_sizes_gb += " " + " ".join([str(st.get("size_gb", config["azure_cluster"]["infra_local_storage_sz"]))] * st["disk_num"])
            elif "worker" in spec["role"]:
                storage_sku += " " + " ".join(["{}={}".format(dsk_id, st.get("sku", config["azure_cluster"][
                    "vm_local_storage_sku"])) for dsk_id in range(disk_id, disk_id + st["disk_num"])])
                data_disk_sizes_gb += " " + " ".join([str(st.get("size_gb", config["azure_cluster"]["worker_local_storage_sz"]))] * st["disk_num"]) 
            elif "nfs" in spec["role"]:
                storage_sku += " " + " ".join(["{}={}".format(dsk_id, st.get("sku", config["azure_cluster"][
                    "nfs_data_disk_sku"])) for dsk_id in range(disk_id, disk_id + st["disk_num"])])
                data_disk_sizes_gb += " " + " ".join([str(st.get("size_gb", config["azure_cluster"]["nfs_data_disk_sz"]))] * st["disk_num"]) 
        disk_id += st["disk_num"]
    else:
        if "infra" in spec["role"]:
            data_disk_sizes_gb += " " + str(config["azure_cluster"]["infra_local_storage_sz"])
            storage_sku = config["azure_cluster"]["vm_local_storage_sku"]
        elif "worker" in spec["role"]:
            data_disk_sizes_gb += " " + str(config["azure_cluster"]["worker_local_storage_sz"])
            storage_sku = config["azure_cluster"]["vm_local_storage_sku"]
        if "nfs" in spec["role"]:
            ...
            storage_sku = storage_sku if "infra" in spec["role"] else config["azure_cluster"]["nfs_data_disk_sku"]
...
Command:
    ...
    parser.add_argument('-o', '--output', default='', help='Specify the output file path')
