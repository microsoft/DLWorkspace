...
def update_config():
    ...
    update_one_config(config, "coreosversion",["coreos","version"], str, coreosversion)
    update_one_config(config, "coreoschannel",["coreos","channel"], str, coreoschannel)
    update_one_config(config, "coreosbaseurl",["coreos","baseurl"], str, coreosbaseurl)
...
def add_additional_cloud_config():
    ...
    translate_config_entry( config, ["coreos", "write_files"], "coreoswritefiles", str, 2 )
    ...
    translate_config_entry( config, ["coreos", "startupScripts"], "startupscripts", str )
...
def get_ETCD_master_nodes_from_cluster_portal(clusterId):
    output = urllib.request.urlopen(form_cluster_portal_URL("etcd", clusterId)).read()
...
def get_worker_nodes_from_cluster_report(clusterId):
    output = urllib.request.urlopen(form_cluster_portal_URL("worker", clusterId)).read()
    output = json.loads(json.loads(output))
    Nodes = []
    NodesInfo = [node for node in output["nodes"] if "time" in node]
    if not "ipToHostname" in config:
...
def run_command( args, command, nargs, parser ):
    ...
    if command =="clean":
        ...
        if len(nargs)<=0:
            ...
            response = input ("Please type (CLEAN) in ALL CAPITALS to confirm the operation ---> ")
            if response == "CLEAN":
