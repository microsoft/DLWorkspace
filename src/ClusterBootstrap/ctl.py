...
def load_config_4_ctl(args, command):
    ...
    if command in ["svc", "render_template", "download", "docker", "db"]:
        args.config = [ENV_CNF_YAML, STATUS_YAML
                      ] if not args.config else args.config
...
def run_script(node,
    ...
    utils.SSH_exec_cmd_with_directory(ssh_cert, adm_usr, node, srcdir, fullcmd,
                                      noSupressWarning)


def run_cmd(node, ssh_cert, adm_usr, nargs, sudo=False, noSupressWarning=True):
    fullcmd = " ".join(nargs)
    utils.SSH_exec_cmd(ssh_cert, adm_usr, node, fullcmd, noSupressWarning)
...
def copy2_wrapper(arg_tuple):
    ...
    if sudo:
        utils.sudo_scp(ssh_cert,
                       source,
                       target,
                       adm_usr,
                       node,
                       verbose=noSupressWarning)
    else:
        utils.scp(ssh_cert,
                  source,
                  target,
...
def execute_in_parallel(config,
                        nodes,
                        nargs,
                        sudo,
                        func,
    ...
    args_list = [(config["machines"][node]["fqdns"], config["ssh_cert"],
                  config["admin_username"], nargs, sudo, noSupressWarning)
                 for node in nodes]
...
def verify_all_nodes_ready(config, args):
    ...
    if len(list(nodes_expected_but_not_ready)) > 0:
        print("following nodes not ready:\n{}".format(','.join(
            list(nodes_expected_but_not_ready))))
        exit(1)
...
def remote_config_update(config, args, check_module=False):
    ...
    if check_module:
        assert set(args.nargs[1:]) - set([
            "storagemanager", "repairmanager", "dashboard"
        ]) == set(), "not supported"
...
def maintain_db(config, args):
    ...
    if subcommand == "connect":
        ...
        if subcommand == "pull":
            ...
            output_file = args.output if args.output else "{}.yaml".format(
                table_name)
            ...
            cursor = conn.cursor()
            cursor.execute(sql)
            input_file_list = args.input if args.input else [
                "{}.yaml".format(table_name)
            ]
            ...
            cols_filtered = [
                col for col in col_names if col not in cols_2_ignore
            ]
            cols_str = ", ".join(cols_filtered)
            for row in table_config["rows"]:
                vals = ", ".join([
                    "'{}'".format(json.dumps(row[col])) for col in cols_filtered
                ])
...
def cordon(config, args):
    ...
    dlts_admin_config_path = config.get("dlts_admin_config_path",
                                        dlts_admin_config_path)
...
def uncordon(config, args):
    node = args.nargs[0]
    query_cmd = "get nodes {} -o=jsonpath=\'{{.metadata.annotations.cordon-note}}\'".format(
        node)
    ...
    if output and not args.force:
        ...
        run_kubectl(config, args,
                    ["annotate node {} cordon-note-".format(node)])
...
def run_command(args, command):
    ...
    if command == "restorefromdir":
        ...
        assert len(
            args.nargs
        ) > 1 and "at least 1 action and 1 kubernetes service name should be provided"
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        action='append',
        default=[],
        help='Specify the config files you want to load, later ones \
    ...
    parser.add_argument('-o',
                        '--output',
                        default='',
                        help='File to dump to as output')
    ...
    parser.add_argument(
        ...
        action='append',
        default=[],
        help='Specify the roles of machines that you want to copy file \
